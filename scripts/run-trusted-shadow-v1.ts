...
function main(): void {
  ...
  const runner = new TrustedShadowV1Runner({
    ...
    transportStaleAfterMs: Number(value('transport-stale-after-ms', '5000')),
    maxPairGapMs: Number(value('max-pair-gap-ms', '2000')),
    discoveryLimit: Number(value('discovery-limit', '50')),
    minimumQualityMarkets: Number(value('minimum-quality-markets', '20')),
    minimumQualityPairs: Number(value('minimum-quality-pairs', '20')),
    minimumQualityCycles: Number(value('minimum-quality-cycles', '2')),
